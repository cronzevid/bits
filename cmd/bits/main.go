...
func percentCalc(timer string) (TickerData, string) {
	...
	if exi {
		tickerF, _ := os.ReadFile("ticker.json")
		_ = json.Unmarshal(tickerF, &tickerFile)
		if timer == "h" {
...
func notifyWrapper(tickerType string, bot *tgbotapi.BotAPI) {
	...
	err := os.WriteFile("ticker.json", fileW, 0644)
	check(err)

	var usersData UserList
	fileR, _ := os.ReadFile("users.json")
	_ = json.Unmarshal(fileR, &usersData.Users)
			...
			if update.Message.IsCommand() {
				...
				case "start":
					...
					if exi {
						...
						usersData = UserList{}
						_ = json.Unmarshal(file, &usersData.Users)
				...
				case "whoami":
					exi, _ := Exists("users/" + UserName + ".json")
					if exi {
						file, _ := os.ReadFile("users/" + UserName + ".json")
					...
					if exi {
						...
						for ind, item := range usersData.Users {
							if item == ChatID {
								...
								err := os.WriteFile("users.json", fileW, 0644)
								check(err)
							}
						}
				...
				if len(regSequence) != registerQuestion {
					...
					err := os.WriteFile("users/"+UserName+".json", file, 0644)
					check(err)
